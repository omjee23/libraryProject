...
const registerMember = async (memberDetail)=> {
    try {
        ...
        const registerData = await dbCollection.findOne({"mobileNumber": memberDetail.mobileNumber});
        if(registerData){
...
const updateMember = async (updateMember) => {
    try {
        const dbconn = await connecToDb();
        const dbCollection = dbconn.collection(dbConstants.membersCollection);
        const updateData = await dbCollection.findOneAndUpdate({"mobileNumber": updateMember.mobileNumber },{$set:updateMember},{returnDocument : "after"});
        return updateData;
    } catch (error) {
...
const deleteMember = async (deleteData) => {
    try {
        const dbconn = await connecToDb();
        const dbCollection = dbconn.collection(dbConstants.membersCollection);
        const deleteMember = await dbCollection.findOneAndDelete({"mobileNumber": deleteData.mobileNumber},{returnDocument : "after"});
        return deleteMember;
    } catch (error) {
